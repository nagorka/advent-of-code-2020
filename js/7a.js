const demo = `light red bags contain 1 bright white bag, 2 muted yellow bags.
...
const rules = input.split("\n").map(row => {
  const [color, bagsStr] = row.split(" bags contain ");
  const bags = bagsStr
    .split(", ")
    .map(x => {
      const m = x.match(/^(\d+)\s(.*)\sbags?(?:\b|\.)/);
      if (!m) return null;
      return { color: m[2], count: parseInt(m[1], 10) };
    })
    .filter(x => x);
  return { color, bags };
});

const allRules = rules.reduce((memo, r) => {
  return {
    ...memo,
    [r.color]: r
  };
...
const getColors = (allRules, rules, color) => {
  return rules.filter(rule => {
    if (rule.color === color) {
      return true;
    }
    const childrenRules = rule.bags.map(r => allRules[r.color]);
    const children = getColors(allRules, childrenRules, color);
    return children.length > 0;
  });
};

const valid = getColors(allRules, rules, "shiny gold");
