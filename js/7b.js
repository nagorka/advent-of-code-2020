...
const getCount = (allRules, color) => {
  const rule = allRules[color];
  return rule.bags.reduce((memo, bag) => {
    return memo + bag.count + bag.count * getCount(allRules, bag.color);
  }, 0);
};

const count = getCount(allRules, "shiny gold");
