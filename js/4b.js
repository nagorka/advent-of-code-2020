const demo = `ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
const d2 = `pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
...
const req = [
  ...
  [
    "ecl",
    x => {
      const ok = "amb blu brn gry grn hzl oth".split(" ").indexOf(x) !== -1;
      //console.log(x, ok);
      return ok;
    }
  ],
...
const valid = input.split("\n\n").filter(data => {
  ...
  const res = pass.reduce((memo, x) => {
    const [key, value] = x.split(":");
    return { ...memo, [key]: value };
  }, {});
  const all = req.every(([key, fn]) => res[key] && fn(res[key]));
  return all;
});
