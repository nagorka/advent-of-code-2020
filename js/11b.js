...
const getFirstVisibleSeat = (seats, row, col, drow, dcol) => {
  let i = row + drow;
  let j = col + dcol;
  while (
    i >= 0 &&
  ...
  return (seats[i] && seats[i][j]) || null;
};

const next = (seats, level) => {
  let c = 0;
  let o = 0;
  console.log("round");
  const n = seats.map((x, row, rows) => {
    return x.map((y, col, cols) => {
      if (y === ".") {
        return y;
      }
      //let adjacent = 0;
      const firstSeats = [
        getFirstVisibleSeat(seats, row, col, -1, -1),
        getFirstVisibleSeat(seats, row, col, -1, 0),
        getFirstVisibleSeat(seats, row, col, -1, 1),
        getFirstVisibleSeat(seats, row, col, 0, -1),
        getFirstVisibleSeat(seats, row, col, 0, 1),
        getFirstVisibleSeat(seats, row, col, 1, -1),
        getFirstVisibleSeat(seats, row, col, 1, 0),
        getFirstVisibleSeat(seats, row, col, 1, 1)
      ...
      const adjacent = firstSeats.reduce((m, s) => {
        if (s === "#") {
          return m + 1;
        }
        return m;
  ...
  if (c > 0) {
    return next(n, level + 1);
  }
  return { level, seats: n, occupied: o };
};

const res = next(seats, 1);

console.log(res.level, res.occupied);
